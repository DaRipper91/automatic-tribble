    ...
    def on_directory_selected(self, event) -> None:
        ...
        if dual:
            ...
            if tabs.active:
                try:
                    pane = tabs.get_pane(tabs.active)
                    # Use internal _label or similar, TabPane label is actually Tab title, Textual doesn't have a direct setter usually,
                    # but maybe setting pane._tab.label? Or just ignore for mypy.
                    pane.label = f"{active_panel.current_dir.name or '/'} [X]" # type: ignore
                except Exception:
                    pass
