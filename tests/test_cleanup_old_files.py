...
class TestCleanupOldFiles:
    ...
    async def test_cleanup_basic(self):
        ...
        # Run cleanup
        deleted_files = await self.organizer.cleanup_old_files(
        ...
        assert new_file not in deleted_files

    async def test_cleanup_recursive(self):
        ...
        # Verify old file is deleted
        assert not old_file.exists(), "Old file in subdir should be deleted"
        assert old_file in deleted_files

        # Verify new file is kept
        assert new_file.exists(), "New file in subdir should be kept"
        assert new_file not in deleted_files

    async def test_cleanup_no_recursive(self):
        # Create directory structure
        subdir = self.test_dir / "subdir"
        subdir.mkdir()
        ...
        assert old_file.exists(), "Old file in subdir should be kept when recursive=False"
        assert old_file not in deleted_files
    ...
    async def test_cleanup_dry_run(self):
        ...
        assert old_file.exists(), "Old file should be kept in dry run"
        assert old_file in deleted_files, "Old file should be reported in dry run"

    async def test_cleanup_edge_case_days(self):
        ...
        assert not zero_days_file.exists(), "File created before now should be deleted with days_old=0"
        assert zero_days_file in deleted_files
